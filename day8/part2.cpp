...
public:
    ...
    Vector2D mirror(Vector2D const & to_mirror, int bounds, int repeat) {
        int delta_x = _x + (_x - to_mirror._x) * repeat;
        int delta_y = _y + (_y - to_mirror._y) * repeat;
                ...
                if (node != to_mirror) {
                    try {
                        for (int repeat = 0;; repeat++) {
                            Vector2D mirrored = node.mirror(to_mirror, bounds, repeat);
